...
/**
 * 文 件 名: CustomStatusView
 * 创 建 人: sineom
 * 创建日期: 2018/12/6 14:53
 * 修改时间：
 * 修改备注： 自定义状态显示
 */
...
class CustomStatusView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) :
    ...
    init {
        ...
        progressColor = array.getColor(
            R.styleable.CustomStatusView_progress_color,
            ContextCompat.getColor(context, R.color.colorPrimary)
        )
        loadSuccessColor = array.getColor(
            R.styleable.CustomStatusView_load_success_color,
            ContextCompat.getColor(context, R.color.load_success)
        )
        loadFailureColor = array.getColor(
            R.styleable.CustomStatusView_load_failure_color,
            ContextCompat.getColor(context, R.color.load_failure)
        )
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        if (mStatus == StatusEnum.Loading) {    //正在加载
            ...
            canvas.drawArc(
                RectF(0f, 0f, progressRadius * 2, progressRadius * 2),
                startAngle.toFloat(),
                sweepAngle.toFloat(),
                false,
                mPaint!!
            ...
            mPathMeasure!!.getSegment(
                0f,
                circleValue * mPathMeasure!!.length,
                mPathCircleDst,
                true
